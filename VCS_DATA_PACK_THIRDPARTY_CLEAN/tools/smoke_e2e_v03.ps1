...
function RunPS([string]$step, [string]$scriptPath, [hashtable]$args) {
...
try {
  RunPS "1/6" $smokeLiveToWS @{ WorkspaceRoot = $ws }
...
try {
  RunPS "2/6"  $applyScenePatch @{ PackDir=$live; MaxScenes=$MaxScenes }
  RunPS "2b/6" $smokeLiveMF     @{ LiveDir=$live; MaxScenes=$MaxScenes }
} catch { $errors += $_.Exception.Message; if ($FailFast) { throw } }

try {
  RunPS "3/6" $applySubsLive @{ LiveDir=$live }
  RunPS "4/6" $smokeSubsLive @{ LiveDir=$live; MaxScenes=$MaxScenes }
} catch { $errors += $_.Exception.Message; if ($FailFast) { throw } }

if ($DoHandoff) {
  try {
    $handoffOut = Join-Path $live "handoff_v03"
    RunPS "5/6" $finalizeHandoff @{ LiveDir=$live; OutDir=$handoffOut }
    ...
    RunPS "6/6" $handoffPack @{ InDir=$handoffOut; OutZip=$zip }
