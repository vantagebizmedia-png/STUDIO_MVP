...
def apply_scene_builder_to_manifest(
    ...
    *,
    ...
    for sc in scenes:
        ...
        provider_selected = _norm_text(r.get("provider_selected")).lower()
        if not provider_selected:
            provider_selected = _norm_text(r.get("provider")).lower()

        provider_detail = _norm_text(r.get("provider_detail"))

        raw_provider_order_used = r.get("provider_order_used")
        provider_order_seen = []
        provider_order_source = raw_provider_order_used if isinstance(raw_provider_order_used, (list, tuple)) else provider_order

        if isinstance(provider_order_source, (list, tuple)):
            for raw_provider_name in provider_order_source:
                normalized_provider_name = _norm_text(raw_provider_name).lower()
                if normalized_provider_name and normalized_provider_name not in provider_order_seen:
                    provider_order_seen.append(normalized_provider_name)
    ...
    if isinstance(provider_order, (list, tuple)):
        for raw_provider_name in provider_order:
            normalized_provider_name = _norm_text(raw_provider_name).lower()
            if normalized_provider_name and normalized_provider_name not in persisted_provider_order:
                persisted_provider_order.append(normalized_provider_name)

    if not persisted_provider_order:
        persisted_provider_order = ["pixabay"]

    manifest["scene_builder_v03"] = {
        "max_scenes": len(scenes),
        "total_audio_ms": int(effective_total_ms),
        "provider_order": persisted_provider_order,
