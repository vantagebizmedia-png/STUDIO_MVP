...
function Ensure-Scenes {
  ...
  # Si ya hay escenas, recorta/usa hasta N
  if ($sc.Count -gt $N) { $sc = $sc[0..($N-1)] }

  # Si faltan, crea escenas nuevas deterministas (tiempos desde audio_clips)
  if ($sc.Count -lt $N) {
    # total_ms basado en último audio_clip
    $lastEnd = 0
    foreach ($c in $m.audio_clips) {
      $e = [int]$c.end_ms
      if ($e -gt $lastEnd) { $lastEnd = $e }
    }
    if ($lastEnd -le 0) { $lastEnd = 20000 } # fallback determinista
...
# --- Asignar texto por escena (simple/determinista) ---
# Si existe m.script o m.text.script, lo repartimos; si no, dejamos vacío.
$scriptText = ""
try {
...
# --- Imagen por escena ---
# Folder assets para escenas dentro del LIVE
...
# fallback image (del artifacts.image)
...
for ($i=0; $i -lt $m.scenes_v03.Count; $i++) {
  ...
  # Si ya tiene image y existe, ok
  $curRel = [string]$scene.assets.image
  if ($curRel) {
    $curRel = $curRel.Replace("/", "\")
    $curAbs = Join-Path $live $curRel
    if (Test-Path -LiteralPath $curAbs) { continue }
  }
  ...
  try {
    ...
    # si pixabay falla, caemos a fallback
    $picked = $null
  }
  ...
  # Fallback: copia determinista del artifacts.image
...
Write-Host ("OK: scene_builder v03 aplicado. scenes={0} live={1}" -f $m.scenes_v03.Count, $live) -ForegroundColor Green
