...
# --- Auto FontSize (determinista) basado en métricas del SRT ---
$FontSizeEffective = $FontSize
try {
  $srtPath = Join-Path $live $SrtName
  $measure = Join-Path $repo "tools\measure_srt_v03.ps1"

  if ((Test-Path -LiteralPath $srtPath) -and (Test-Path -LiteralPath $measure)) {
    $metricsJson = pwsh -NoProfile -ExecutionPolicy Bypass -File $measure -SrtPath $srtPath
    $metrics = $metricsJson | ConvertFrom-Json

    $maxChars = [int]$metrics.max_chars_line
    $maxLines = [int]$metrics.max_lines_block

    if     ($maxChars -gt 46) { $FontSizeEffective = 36 }
    elseif ($maxChars -gt 40) { $FontSizeEffective = 40 }
    elseif ($maxChars -gt 34) { $FontSizeEffective = 44 }
    elseif ($maxChars -gt 28) { $FontSizeEffective = 48 }

    if ($maxLines -ge 3) { $FontSizeEffective = $FontSizeEffective - 4 }
    if ($FontSizeEffective -lt 34) { $FontSizeEffective = 34 }

    Write-Host "AUTO-FONT: maxChars=$maxChars maxLines=$maxLines FontSize=$FontSize -> $FontSizeEffective" -ForegroundColor DarkGray
  } else {
    Write-Host "AUTO-FONT: skip (no SRT o no measure_srt_v03.ps1)" -ForegroundColor DarkGray
  }
} catch {
  Write-Host "AUTO-FONT: skip por error" -ForegroundColor DarkGray
}
# --- fin auto-font ---
...
pwsh -NoProfile -ExecutionPolicy Bypass -File $applySubs `
  -PackDir $live `
  -SrtName $SrtName `
  -FontSize $FontSizeEffective -MarginV $MarginV -Outline $Outline
