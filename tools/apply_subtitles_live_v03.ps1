...
try {
  ...
  if (Test-Path -LiteralPath $srtPath -and (Test-Path -LiteralPath $measure)) {
    ...
    Write-Host ("AUTO-FONT: maxChars={0} maxLines={1} FontSize={2} -> {3}" -f $maxChars, $maxLines, $FontSize, $FontSizeEffective) -ForegroundColor DarkGray
  } else {
    Write-Host "AUTO-FONT: skip (no SRT o no measure_srt_v03.ps1)" -ForegroundColor DarkGray
  }
} catch {
  Write-Host ("AUTO-FONT: skip por error: {0}" -f param(
  [string]$LiveDir = "",
  [string]$SrtName = "captions_v03.srt",
  [int]$FontSize = 52,
  [int]$MarginV  = 120,
