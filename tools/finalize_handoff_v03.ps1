param(
  [Parameter(Mandatory=$false)][string]$LiveDir,
  [Parameter(Mandatory=$false)][string]$WorkspaceRoot,
  [string]$HandoffDirName = "handoff_v03",
  [switch]$Force
)

Set-StrictMode -Version Latest
$ErrorActionPreference = "Stop"

$psUtf8Compat = Join-Path $PSScriptRoot "ps_utf8_compat_v03.ps1"
if (-not (Test-Path -LiteralPath $psUtf8Compat -PathType Leaf)) {
  throw ("No existe helper utf8 compat: {0}" -f $psUtf8Compat)
}

. $psUtf8Compat

$repo = (Resolve-Path ".").Path

function Ensure-Dir([string]$p) {
  if (-not (Test-Path -LiteralPath $p)) {
    New-Item -ItemType Directory -Force -Path $p | Out-Null
  }
}

function Safe-Remove([string]$p) {
  if (Test-Path -LiteralPath $p) {
    Remove-Item -LiteralPath $p -Force -Recurse
  }
}

function Write-Hashes([string]$dir, [string]$outFile) {
  $files = Get-ChildItem -LiteralPath $dir -File -Recurse | ForEach-Object {
    $full = $_.FullName
    $rel  = $full.Substring($dir.Length).TrimStart("\","/") -replace "\\","/"
    $relLower = $rel.ToLowerInvariant()

    if ($rel -match '(^|/)\.tmp_') { return }
    if ($relLower.EndsWith(".zip")) { return }
    if ($relLower -eq "handoff_ready.txt") { return }
    if ($relLower -eq "hashes_sha256.txt") { return }

    [pscustomobject]@{
      Full = $full
      Rel  = $rel
    }
  } | Where-Object { $_ -ne $null } | Sort-Object Rel

  $lines = New-Object System.Collections.Generic.List[string]
  foreach ($f in $files) {
    $h = (Get-FileHash -LiteralPath $f.Full -Algorithm SHA256).Hash.ToLowerInvariant()
    $lines.Add(("{0}  {1}" -f $h, $f.Rel))
  }

  $utf8NoBom = New-Object System.Text.UTF8Encoding($false)
  [System.IO.File]::WriteAllText($outFile, (($lines -join "`n") + "`n"), $utf8NoBom)
}

if (-not $LiveDir -or $LiveDir.Trim().Length -eq 0) {
  if (-not $WorkspaceRoot -or $WorkspaceRoot.Trim().Length -eq 0) {
    throw "Falta -LiveDir o -WorkspaceRoot"
  }
  $LiveDir = Join-Path $WorkspaceRoot "runs\smoke_live_latest"
}

$live = (Resolve-Path $LiveDir).Path
if (-not (Test-Path -LiteralPath $live)) { throw "No existe LIVE: $live" }

$ensure = Join-Path $repo "tools\ensure_outputs_live_v03.ps1"
if (-not (Test-Path -LiteralPath $ensure)) { throw "Falta tool: $ensure" }

pwsh -NoProfile -ExecutionPolicy Bypass -File $ensure -LiveDir $live | Out-Null
$ensureExit = $LASTEXITCODE
if ($ensureExit -ne 0) {
  throw "ensure_outputs_live_v03.ps1 devolvió exit code $ensureExit"
}

$video          = Join-Path $live "video.mp4"
$musicAuto      = Join-Path $live "video_music_auto.mp4"
$final          = Join-Path $live "video_final.mp4"
$videoSubtitles = Join-Path $live "video_subtitles.mp4"
$videoSubs      = Join-Path $live "video_subs.mp4"
$srt            = Join-Path $live "captions_v03.srt"

if (-not (Test-Path -LiteralPath $video))          { throw "Falta output: $video" }
if (-not (Test-Path -LiteralPath $musicAuto))      { throw "Falta output: $musicAuto" }
if (-not (Test-Path -LiteralPath $final))          { throw "Falta output: $final" }
if (-not (Test-Path -LiteralPath $videoSubtitles)) { throw "Falta output: $videoSubtitles" }
if (-not (Test-Path -LiteralPath $videoSubs))      { throw "Falta output: $videoSubs" }
if (-not (Test-Path -LiteralPath $srt))            { throw "Falta output: $srt" }

$handoffDir = Join-Path $live $HandoffDirName
if ($Force) { Safe-Remove $handoffDir }
Ensure-Dir $handoffDir

Copy-Item -LiteralPath $video          -Destination (Join-Path $handoffDir "video.mp4")            -Force
Copy-Item -LiteralPath $musicAuto      -Destination (Join-Path $handoffDir "video_music_auto.mp4") -Force
Copy-Item -LiteralPath $final          -Destination (Join-Path $handoffDir "video_final.mp4")      -Force
Copy-Item -LiteralPath $videoSubtitles -Destination (Join-Path $handoffDir "video_subtitles.mp4")  -Force
Copy-Item -LiteralPath $videoSubs      -Destination (Join-Path $handoffDir "video_subs.mp4")       -Force
Copy-Item -LiteralPath $srt            -Destination (Join-Path $handoffDir "captions_v03.srt")     -Force

$hashes = Join-Path $handoffDir "HASHES_SHA256.txt"
$ready  = Join-Path $handoffDir "HANDOFF_READY.txt"

Write-Hashes -dir $handoffDir -outFile $hashes

$utf8NoBom = New-Object System.Text.UTF8Encoding($false)
$readyText = @()
$readyText += "HANDOFF_READY v0.3"
$readyText += "FILES:"
$readyText += "- video.mp4"
$readyText += "- video_music_auto.mp4"
$readyText += "- video_final.mp4"
$readyText += "- video_subtitles.mp4"
$readyText += "- video_subs.mp4"
$readyText += "- captions_v03.srt"
$readyText += "NOTES:"
$readyText += "- video.mp4 = render base sin música."
$readyText += "- video_subs.mp4 / video_subtitles.mp4 = base con burn-in."
$readyText += "- video_music_auto.mp4 = salida con música automática real si existe music.wav o music.mp3; si no, fallback desde video_subs.mp4."
$readyText += "- video_final.mp4 = salida final entregable."
$readyText += "- HASHES_SHA256.txt contiene hashes solo de entregables."
$readyText += "- HANDOFF_READY.txt, HASHES_SHA256.txt y handoff_v03.zip quedan fuera del hash set."

[System.IO.File]::WriteAllText($ready, (($readyText -join "`n") + "`n"), $utf8NoBom)

Write-Host "OK: finalize_handoff_v03 -> $handoffDir (READY/HASHES sin circularidad y sin ZIP)" -ForegroundColor Green
